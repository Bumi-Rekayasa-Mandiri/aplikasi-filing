...
class SuratController extends Controller
{
    ...
    public function show(Surat $surat)
    {
        return Inertia::render('filing/surat/Show', [
            'surat' => [
                ...
                'meta'           => $surat->meta,
                'last_regenerated_at' => $surat->last_regenerated_at?->toIso8601String(),
    ...
    public function edit(Surat $surat, string $jenis)
    {
        ...
        // Eager load TTDs (urut)
        $surat->load(['ttds' => fn($q) => $q->orderBy('urutan')]);
    
        // ── URL Cap ───────────────────────────────────────────────────────────
        $capMedia = $surat->getFirstMedia('cap');
        $capUrl   = $capMedia ? $capMedia->getUrl() : null;
    
        // ── Bangun TTDs manual (tanpa toArray) ───────────────────────────────
        $ttdsData = $surat->ttds->map(function ($ttd) {
            $ttdMedia = $ttd->getFirstMedia('ttd');
            return [
                'id'                  => $ttd->id,
                'nama_penandatangan'  => $ttd->nama_penandatangan,
                'jabatan'             => $ttd->jabatan,
                'label'               => $ttd->label,
                'urutan'              => $ttd->urutan,
                'ttd_url'             => $ttdMedia ? $ttdMedia->getUrl() : null,
            ];
        })->values()->toArray();
        ...
        $ktpUrls = [];
        if ($jenis === 'BRM-1') {
            $ktpUrls = $surat->getMedia('ktp')
                ->map(fn($m) => $m->getUrl())
                ->values()
                ->toArray();
        }
    
        // ── Bangun payload surat manual (eksplisit, aman, tidak trigger appends) ──
        $suratPayload = [
            'id'                => $surat->id,
            'jenis'             => $surat->jenis,
            'nomor_surat'       => $surat->nomor_surat,
            ...
            // Field per jenis (semua kolom di tabel surat — tidak masalah jika null)
            'nama'              => $surat->nama,
            'jabatan_terakhir'  => $surat->jabatan_terakhir,
            'departemen'        => $surat->departemen,
            'lokasi_kerja'      => $surat->lokasi_kerja,
            ...
            // Meta (sudah di-cast ke array oleh model)
            'meta'              => $surat->meta ?? [],
            'last_regenerated_at' => $surat->last_regenerated_at?->toIso8601String(),
    ...
    public function update(
            Request $request,
            Surat $surat,
            RegenerateSuratFiles $regenerator
        ) {
            $this->authorize('update', $surat);
    
            // ── GATE: Block edit jika status approved ────────────────────────
            $statusValue = is_object($surat->status) ? $surat->status->value : $surat->status;
            if ($statusValue === 'approved') {
                return back()->withErrors([
                    'status' => 'Surat dengan status approved tidak dapat diedit. Revert ke draft terlebih dahulu.',
                ]);
            }
            ...
            DB::transaction(function () use ($request, $surat, $validated) {
                $this->updateCoreFields($surat, $validated);
    
                if ($request->hasFile('cap')) {
                    $surat->addMedia($request->file('cap'))
                        ->toMediaCollection('cap');
                }
    
                $this->syncTtds($surat, $request);
                $this->updateMeta($surat, $request);
    
                if ($surat->jenis === 'BRM-1') {
                    $this->syncPekerjaKtp($surat, $request);
                }
            });
    
            // ── AUTO-REGENERATE PDF/DOCX (only for draft) ────────────────────
            // Refresh model agar dapat data terbaru (meta, relasi ttds, media)
            $surat->refresh();
            $surat->load('ttds');
    
            $regenResult = $regenerator->handle($surat);
    
            // ── BUILD FLASH MESSAGE ──────────────────────────────────────────
            $message    = 'Surat berhasil diperbarui';
            $messageKey = 'success';
    
            if (!$regenResult['pdf'] && !$regenResult['docx']) {
                // Data tersimpan tapi regenerate gagal total
                $message    = 'Data surat tersimpan, namun PDF & DOCX gagal di-regenerate. Silakan hubungi admin.';
                $messageKey = 'warning';
            } elseif (!$regenResult['pdf'] || !$regenResult['docx']) {
                // Sebagian gagal
                $failed     = !$regenResult['pdf'] ? 'PDF' : 'DOCX';
                $message    = "Surat berhasil diperbarui, namun {$failed} gagal di-regenerate.";
                $messageKey = 'warning';
            } else {
                // Semua berhasil
                $message = 'Surat berhasil diperbarui. PDF & DOCX telah di-regenerate ulang.';
            }
        ...
        private function buildValidationRules(string $jenis, ?Surat $surat = null): array
        {
            $base = [
                ...
                'ttds.*.file'                => 'nullable|file|image|max:2048',
            ];
    
            $perJenis = match ($jenis) {
                'BRM-1' => [
                    ...
                    'pekerja.*.role'    => 'nullable|string|max:50',
                    'pekerja.*.ktp'     => 'nullable|file|image|max:2048',
                ],
                'BRM-2' => [
            ...
            return array_merge($base, $perJenis);
        }
    
        // ══════════════════════════════════════════════════════════════════════
        // 1. UPDATE FIELD UTAMA — exclude relasi dan media
        // ══════════════════════════════════════════════════════════════════════
        private function updateCoreFields(Surat $surat, array $validated): void
        {
            $excluded = ['cap', 'ttds', 'jsa', 'pekerja', 'dokumen'];
            $fillable = array_diff_key($validated, array_flip($excluded));
    
            $surat->update($fillable);
        }
    
        // ══════════════════════════════════════════════════════════════════════
        // 2. SYNC TTDs — upsert + delete
        // ══════════════════════════════════════════════════════════════════════
        private function syncTtds(Surat $surat, Request $request): void
        {
            if (!$request->has('ttds')) {
                return; // user tidak edit TTDs
            }
    
            $ttdsInput = $request->input('ttds', []);
    
            // ID TTD yang masih ada di payload
            $keepIds = collect($ttdsInput)
                ->pluck('id')
                ->filter()
                ->map(fn($v) => (int) $v)
                ->toArray();
    
            // Hapus TTD yang tidak lagi ada (cascade delete media via model event)
            $surat->ttds()
            ...
            foreach ($ttdsInput as $idx => $ttdData) {
                ...
                if (!empty($ttdData['id'])) {
                    $ttd = $surat->ttds()->where('id', $ttdData['id'])->first();
                    if ($ttd) {
                        $ttd->update($payload);
                    }
                } else {
                    $ttd = $surat->ttds()->create($payload);
                }
    
                // File upload per index — pakai helper trait
                $file = $this->getNestedFile($request, 'ttds', $idx, 'file');
                if ($file && $ttd) {
                    // Tambah media baru — collection 'ttd' biasanya singleFile,
                    // jadi otomatis replace yang lama
                    $ttd->addMedia($file)->toMediaCollection('ttd');
                }
            }
        }
    
        // ══════════════════════════════════════════════════════════════════════
        // 3. UPDATE META — jsa, pekerja, dokumen
        // ══════════════════════════════════════════════════════════════════════
        private function updateMeta(Surat $surat, Request $request): void
        {
            $meta = $surat->meta ?? [];
            ...
            if ($request->has('pekerja')) {
                $meta['pekerja'] = collect($request->input('pekerja', []))
                    ->map(fn($item) => [
                        'nama' => $item['nama'] ?? '',
                        'role' => $item['role'] ?? 'Pekerja',
                    ])
                    ->values()
                    ->toArray();
            }
            ...
            if ($request->has('dokumen')) {
                $meta['dokumen'] = array_values(array_filter(
                    $request->input('dokumen', []),
                    fn($v) => is_string($v) && trim($v) !== ''
                ));
            }
    
            $surat->update(['meta' => $meta]);
        }
    
        // ══════════════════════════════════════════════════════════════════════
        // 4. SYNC PEKERJA KTP (BRM-1)
        // ══════════════════════════════════════════════════════════════════════
        private function syncPekerjaKtp(Surat $surat, Request $request): void
        {
            if (!$request->has('pekerja')) {
                return;
            }
    
            $pekerjaInput  = $request->input('pekerja', []);
            $existingMedia = $surat->getMedia('ktp')->values();
    
            foreach ($pekerjaInput as $idx => $pekerjaData) {
                $newKtpFile = $this->getNestedFile($request, 'pekerja', $idx, 'ktp');
    
                if ($newKtpFile) {
                    // Hapus media lama di posisi ini
                    if ($existingMedia->has($idx)) {
                        $existingMedia->get($idx)->delete();
                    }
                    $surat->addMedia($newKtpFile)
                        ->withCustomProperties(['pekerja_index' => $idx])
                        ->toMediaCollection('ktp');
                }
            }
    
            // Cleanup: hapus media untuk pekerja yang dihapus dari list
            $jumlahPekerja = count($pekerjaInput);
            $existingMedia->slice($jumlahPekerja)->each(fn($m) => $m->delete());
        }
