...
export default function CreateSPI({ surat }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    judul: '', perihal: '', tujuan: '', isi_surat: '', tanggal_surat: '',
    nama: '', alamat: '', no_ktp: '', nominal: '', nominal_bagihasil: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!surat?.id) return
    post(route('filing.surat.generateSPI-pdf', { surat: surat.id }), {
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Buat Surat Permohonan Investasi" />
      <form onSubmit={submit} className="create-form">
        <div className="create-form-heading">
          <h1 className="create-form-title">Form data surat permohonan investasi</h1>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field col-span-2">
              ...
              <input className="field-input" placeholder="Judul surat" onChange={e => setData('judul', e.target.value)} />
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field">
              <label className="field-label">Nama investor</label>
              <input className="field-input" placeholder="Nama lengkap investor"
                onChange={e => setData('nama', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">No KTP</label>
              <input className="field-input" placeholder="Nomor KTP"
                onChange={e => setData('no_ktp', e.target.value)} />
            </div>
            <div className="field col-span-2">
              <label className="field-label">Alamat</label>
              <input className="field-input" placeholder="Alamat lengkap"
                onChange={e => setData('alamat', e.target.value)} />
            </div>
            <div className="field col-span-2">
              <label className="field-label">Isi surat</label>
              <input className="field-input" placeholder="Uraian isi surat"
        ...
        <div className="form-card">
          <div className="form-card-header">
            <div className="form-icon icon-teal">
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                <path d="M2 13l3-3 3 3 5-7" stroke="#0F6E56" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <span className="form-card-title">Detail investasi</span>
          </div>
          <div className="form-grid">
            <div className="field">
              <label className="field-label">Nominal investasi</label>
              <input className="field-input" placeholder="Contoh: Rp 100.000.000"
                onChange={e => setData('nominal', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Nominal bagi hasil</label>
              <input className="field-input" placeholder="Contoh: Rp 5.000.000 / bulan"
                onChange={e => setData('nominal_bagihasil', e.target.value)} />
            </div>
          </div>
        </div>
