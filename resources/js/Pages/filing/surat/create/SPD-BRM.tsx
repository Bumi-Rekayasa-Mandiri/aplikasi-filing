...
export default function CreateSPD({ surat }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    judul: '', perihal: '', lampiran: '', tujuan: '', tanggal_surat: '',
    nama: '', alamat: '', item_pembelian: '', nominal: '', isi_surat: '', no_ktp: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!surat?.id) return
    post(route('filing.surat.generateSPD-pdf', { surat: surat.id }), {
    ...
    <AuthenticatedLayout>
      ...
      <form onSubmit={submit} className="create-form">
        <div className="create-form-heading">
          <h1 className="create-form-title">Form data surat pengembalian dana</h1>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            ...
            <div className="field">
              ...
              <input className="field-input" placeholder="Contoh: 1 lembar" onChange={e => setData('lampiran', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Hal</label>
            ...
            <div className="field col-span-2">
              ...
              <input className="field-input" placeholder="Alamat lengkap" onChange={e => setData('alamat', e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field col-span-2">
              <label className="field-label">Item pembelian</label>
              <input className="field-input" placeholder="Nama item pembelian"
                onChange={e => setData('item_pembelian', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Nominal</label>
              <input className="field-input" placeholder="Contoh: Rp 10.000.000"
                onChange={e => setData('nominal', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Nama bank</label>
              <input className="field-input" placeholder="Nama bank"
                onChange={e => setData('nama', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Cabang</label>
              <input className="field-input" placeholder="Cabang bank"
                onChange={e => setData('isi_surat', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Nomor rekening</label>
              <input className="field-input" placeholder="Nomor rekening"
