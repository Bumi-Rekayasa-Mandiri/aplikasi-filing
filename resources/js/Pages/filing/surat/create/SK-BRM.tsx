...
export default function CreateSK({ surat }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    ...
    hasil_denda: '', keringanan_denda: '', isi_surat: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!surat?.id) return
    post(route('filing.surat.generateSK-pdf', { surat: surat.id }), {
    ...
    <AuthenticatedLayout>
      ...
      <form onSubmit={submit} className="create-form">
        <div className="create-form-heading">
          <h1 className="create-form-title">Form data surat permohonan keringanan denda</h1>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field">
              <label className="field-label">Hasil perhitungan denda</label>
              <input className="field-input" placeholder="Contoh: Rp 5.000.000"
                onChange={e => setData('hasil_denda', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Keringanan denda</label>
              <input className="field-input" placeholder="Contoh: Rp 2.500.000"
                onChange={e => setData('keringanan_denda', e.target.value)} />
            </div>
            <div className="field col-span-2">
              <label className="field-label">Isi surat</label>
              <textarea className="field-input" placeholder="Uraian isi surat"
