...
export default function CreateSKP({ surat }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    ...
    nama: '', jabatan_terakhir: '', departemen: '', isi_surat: '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!surat?.id) return
    post(route('filing.surat.generateSKP-pdf', { surat: surat.id }), {
    ...
    <AuthenticatedLayout>
      ...
      <form onSubmit={submit} className="create-form">
        <div className="create-form-heading">
          <h1 className="create-form-title">Form data surat pemberitahuan PHK</h1>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field">
              ...
              <input className="field-input" placeholder="Nama lengkap karyawan"
                onChange={e => setData('nama', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Jabatan terakhir</label>
              <input className="field-input" placeholder="Jabatan terakhir"
                onChange={e => setData('jabatan_terakhir', e.target.value)} />
            </div>
            <div className="field">
              <label className="field-label">Departemen</label>
              <input className="field-input" placeholder="Nama departemen"
                onChange={e => setData('departemen', e.target.value)} />
            </div>
            <div className="field col-span-2">
              <label className="field-label">Isi surat</label>
              <textarea className="field-input" placeholder="Uraian isi surat PHK"
